 ...
 *
 * Signal mechanism: scripts append their request ID to /ipc/_queue
 * so the JS bridge can discover new requests by reading that single file
 * via ipcDevice.readFileAsBlob() — NO cx.run() needed during polling.
 */
...
export const CURL_BRIDGE_SCRIPT = [
  ...
  'REQEOF',
  '',
  '# Signal request is ready by appending ID to the queue file',
  'echo "$REQ_ID" >> /ipc/_queue',
  '',
  ...
  '  if [ -f "$STOP_FILE" ]; then',
  '    echo \'{"error":{"message":"agent stopped"}}\' >&2',
  '    rm -f "/ipc/${REQ_ID}.json" "$BODY_FILE" 2>/dev/null',
  '    exit 1',
  '  fi',
  '',
  ...
  '    fi',
  '    # Cleanup',
  '    rm -f "/ipc/${REQ_ID}.json" "$BODY_FILE" \\',
...
export const CAST_BRIDGE_SCRIPT = [
  ...
  'REQEOF',
  '',
  '# Signal ready by appending ID to the queue file',
  'echo "$REQ_ID" >> /ipc/_queue',
  '',
  ...
  '  if [ -f "$STOP_FILE" ]; then',
  '    echo "Error: agent stopped" >&2',
  '    rm -f "/ipc/${REQ_ID}.json" 2>/dev/null',
  '    exit 1',
  '  fi',
  '',
  '  if [ -f "/data/resp_${REQ_ID}.ready" ]; then',
